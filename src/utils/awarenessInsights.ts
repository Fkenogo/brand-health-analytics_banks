...
export function buildAwarenessMetricInsight(args: AwarenessMetricInsightArgs): AwarenessInsightResult | null {
  ...
  switch (key) {
    case 'top_of_mind': {
      ...
      const snapshot =
        tier === 'leader' ? 'Market leader in first-recall — this brand anchors the category and holds a durable first-recall advantage that strengthens over time.'
      ...
      const competitive =
        tier === 'leader' ? `COMPETITIVE CONTEXT: First-recall leadership means the brand is the default choice when consumers face a decision without deliberation. This translates into lower cost-per-acquisition, higher word-of-mouth amplification, and resilience during media-dark periods — structural benefits that compound and become progressively harder for challengers to reverse.`
        : tier === 'strong' ? `COMPETITIVE CONTEXT: At this level the brand wins roughly 1 in 5 first-mention moments — meaningful but not dominant. The gap to leadership is within strategic reach: 2-3 focused distinctiveness-led campaign cycles could close it. Without continued momentum, this position is contestable by a challenger brand that increases investment.`
        : tier === 'moderate' ? `COMPETITIVE CONTEXT: A ${value.toFixed(1)}% position means the brand wins approximately 1 in 8 first-mention moments. Competitors holding 20%+ have a structural recall advantage — consumers default to them without deliberation. Breaking into that group requires building stronger brand distinctiveness, not simply increasing media reach.`
        : `COMPETITIVE CONTEXT: At this salience level the brand is not winning first-mention moments in any significant segment. It is effectively a "known option" that only enters consideration when consumers are actively comparing, not instinctively at the moment of decision — a fundamentally weaker commercial position.`;

      const consumer =
        tier === 'leader' || tier === 'strong'
        ? `CUSTOMER BEHAVIOUR: Consumers who name this brand first are demonstrating habitual salience — it has been encoded as a primary category cue. This level of mental availability predicts consistent usage rates, lower churn susceptibility, and reduced sensitivity to competitor promotional offers — the hallmarks of deep brand preference.`
        : tier === 'moderate'
        ? `CUSTOMER BEHAVIOUR: For every consumer who recalls this brand first, 7-8 others recall a competitor — and those consumers default to their top-of-mind brand. This represents a cohort of latent sympathisers: aware, potentially interested, but not yet holding this brand as their primary category anchor. They are winnable at relatively low cost with the right distinctiveness investment.`
      ...
      const risk =
        tier === 'leader' ? `KEY RISK AREAS: Top-of-mind leadership erodes faster than it is built; complacency during the defence phase allows challengers to close the gap quickly. The recall advantage can also support premium positioning and reduce dependence on promotional mechanics that erode brand value long-term.`
        : tier === 'strong' ? `KEY RISK AREAS: Without continued investment, this position can slip to moderate within 1-2 competitor campaign cycles. At ${value.toFixed(1)}%, the brand is close to the threshold where organic recall compounds; the next 5-8pp gain is the highest-value investment in the current planning cycle.`
        : tier === 'moderate' ? `KEY RISK AREAS: A sustained competitor campaign over 2-3 months can absorb the marginal first-recall consumers currently split between brands at this tier. Moving to 20%+ is achievable within 12-18 months with focused distinctiveness investment; above that threshold organic compounding begins.`
        : `KEY RISK AREAS: Without active distinctiveness investment, further first-recall erosion is likely as competitors capture available first-mention share. The aware base already exists; converting even a small share of passive recognisers to spontaneous recall would produce measurable top-of-mind gains quickly.`;

      const strategic =
        tier === 'leader' ? `BUSINESS IMPLICATION: The priority shifts from growing top-of-mind to defending and deepening it. Awareness quality (top-of-mind ÷ total awareness) and spontaneous recall breadth are the next focus metrics — ensuring first-recall leadership translates into full-funnel advantage rather than headline share alone.`
        : tier === 'strong' ? `BUSINESS IMPLICATION: The growth agenda should focus narrowly on crossing the 25-30% leadership threshold. This is more valuable than broad awareness growth — the compounding effects above 25% create significantly better ROI on media investment and build defensible brand equity.`
        : tier === 'moderate' ? `BUSINESS IMPLICATION: The brand faces a middle-tier trap — awareness investment generates reach without building durable salience. Continued broad reach investment without improving brand distinctiveness will grow aided recognition while leaving top-of-mind largely unchanged. The next planning cycle must make distinctiveness, not reach, the primary investment thesis.`
        : `BUSINESS IMPLICATION: Building brand memory structures is the prerequisite for all other metric improvement. Salience cannot be purchased through reach alone — it requires consistent, distinctive, and emotionally resonant creative that encodes specific brand triggers in consumer memory.`;

      const action =
        tier === 'leader' ? `RECOMMENDED NEXT STEPS: Protect share of voice above the current top-of-mind percentage. Monitor awareness quality quarterly for signs of passive decay. Invest in brand asset consistency — visual, audio, and brand cues must remain stable and distinctive to maintain first-recall strength.`
        : tier === 'strong' ? `RECOMMENDED NEXT STEPS: Identify the consumer segment where top-of-mind is most contestable from the current leader and build a focused campaign to win it. Run 2-3 consecutive brand distinctiveness-led cycles with increased frequency. Set the 25% threshold as a specific media and creative objective with a defined timeline.`
        : tier === 'moderate' ? `RECOMMENDED NEXT STEPS: Audit which brand assets — visual identity, audio cues, brand character, tagline — most reliably generate first-recall in this category. Concentrate creative investment on those assets for 2-3 consecutive campaign cycles. Prioritise depth and frequency of impression over breadth of reach.`
        : `RECOMMENDED NEXT STEPS: Begin with a brand distinctiveness sprint: identify the single brand asset with the highest potential for driving first-recall, and build a focused 6-month campaign around it before expanding to broader awareness investment.`;
    ...
    case 'spontaneous_recall': {
      ...
      const competitive = `COMPETITIVE CONTEXT: ${tier === 'excellent' || tier === 'good' ? 'High spontaneous recall provides competitive insulation: even when a competitor outspends significantly on media, the brand retains strong unaided entry into consideration sets. This is one of the hardest forms of brand equity to erode quickly.' : 'Competitors with higher spontaneous recall enjoy a structural advantage in every purchase context where advertising is not present. Each routine financial decision made without active comparison defaults to deeper-recall brands — a persistent conversion disadvantage.'}`;

      const consumer = `CUSTOMER BEHAVIOUR: ${tier === 'excellent' || tier === 'good' ? 'Consumers who recall a brand spontaneously are more likely to recommend it, return to it without incentive, and resist switching under competitor pressure. High spontaneous recall is both a cause and a symptom of loyalty.' : 'The current consumer relationship is primarily recognitional: aware consumers know the name but haven\'t encoded it as a first-choice association. Converting passive recognisers requires deeper, more emotionally resonant brand touchpoints — not more reach, but more resonance.'}`;

      const risk = `KEY RISK AREAS: ${tier === 'excellent' || tier === 'good' ? 'Spontaneous recall can erode during periods of media inactivity or brand inconsistency; brand associations weaken without regular reinforcement. Strong recall also provides headroom to shift investment from awareness-building to preference and loyalty conversion.' : 'Aided-only recognition is fragile: it doesn\'t persist through media-dark periods, generates no word-of-mouth, and offers no competitive protection. The total aware base exists; converting it to spontaneous recall costs far less than building new awareness from scratch.'}`;

      const strategic = `BUSINESS IMPLICATION: ${tier === 'excellent' || tier === 'good' ? 'With strong spontaneous recall, focus should shift to converting that recall into preference and advocacy. Investment should emphasise brand depth: distinctive experiences, service excellence, and advocacy programs that reinforce brand associations already built.' : 'Building spontaneous recall must be the primary brand equity objective. This requires consistent, high-frequency exposure to the same distinctive brand elements — repetition of the same brand cues is essential for building recall.'}`;

      const action = `RECOMMENDED NEXT STEPS: ${tier === 'excellent' || tier === 'good' ? 'Maintain creative consistency — the specific visual, audio, and brand cues that built strong recall must remain stable. Avoid brand refreshes that disrupt established recall. Track spontaneous recall quarterly to catch early erosion signals.' : 'Identify the 2-3 brand cues that most strongly associate with the brand and concentrate investment on embedding them consistently across channels. Consistent high-frequency repetition builds the recall associations needed for spontaneous recall.'}`;
    ...
    case 'total_awareness': {
      ...
      const competitive = `COMPETITIVE CONTEXT: ${tier === 'dominant' || tier === 'strong' ? 'High or near-saturated total awareness is a genuine competitive moat. New market entrants and challengers face substantial investment hurdles to reach the same level — building awareness from 0% to 70%+ in a contested market takes years and significant capital.' : 'Most established competitors hold a structural awareness advantage. The brand is fighting to be known before it can fight to be chosen — competitive parity requires sustained investment before the awareness base can support salience-building campaigns.'}`;

      const consumer = `CUSTOMER BEHAVIOUR: ${tier === 'dominant' || tier === 'strong' ? 'With near-universal or high awareness, the consumer journey starts from familiarity. The quality of awareness becomes the differentiator: consumers who know the brand superficially (aided-only) behave very differently from those who hold it in active recall.' : 'Many consumers in this market have not yet formed any relationship with the brand. Every awareness-building touchpoint is an investment in creating the foundation for future salience, preference, and loyalty.'}`;

      const risk = `KEY RISK AREAS: ${tier === 'dominant' || tier === 'strong' ? 'Total awareness without salience quality is a hollow metric; a brand at 80% total awareness with 10% top-of-mind is at risk of quality decay — known but not chosen. The aware base is also the most cost-efficient pool for salience conversion; retargeting costs far less than building new awareness.' : 'The current reach base is insufficient to generate the organic word-of-mouth needed for self-sustaining growth. Reach investment at this stage has the highest ROI in the brand lifecycle, as it unlocks all downstream metrics simultaneously.'}`;

      const strategic = `BUSINESS IMPLICATION: ${tier === 'dominant' || tier === 'strong' ? 'Awareness quality becomes the primary KPI — specifically, how much of the total aware base is convertible to spontaneous and top-of-mind recall. Media and creative investment should optimise for salience depth rather than raw reach.' : 'Reach expansion is the single highest-priority investment. All other brand KPIs — top-of-mind, spontaneous recall, intent, preference — are capped by total awareness. Every percentage point of awareness growth unlocks a proportional pool of potential salience converters.'}`;

      const action = `RECOMMENDED NEXT STEPS: ${tier === 'dominant' || tier === 'strong' ? 'Track the awareness quality ratio (top-of-mind ÷ total awareness) quarterly. Reduce investment in pure reach campaigns and redirect toward salience-building creative — distinctiveness investment, not more impressions.' : 'Run high-reach awareness campaigns before any category-specific messaging. Prioritise channels with the highest incremental reach among the currently unaware population. Set 60% total awareness as the first milestone before transitioning to salience-building investment mode.'}`;
    ...
    case 'awareness_quality': {
      ...
      const market = `MARKET INTERPRETATION: At ${value.toFixed(1)}%, ${tier === 'excellent' ? 'a large proportion of aware consumers hold active recall of this brand. This quality ratio (top-of-mind ÷ total awareness) measures how "monetisable" awareness is — at 40%+, the brand\'s reach converts efficiently into spontaneous consideration.' : tier === 'good' ? 'a meaningful share of aware consumers actively recall the brand without prompting. This quality level indicates that brand investment is producing real salience, not just name familiarity — a critical distinction in how much organic consideration the brand generates.' : 'the brand has a recognition-salience gap: most aware consumers recognise the name when shown it, but few retrieve it without prompting. This "wide-but-shallow" profile is common among brands that have invested in reach without building distinctive brand cues.'}`;

      const competitive = `COMPETITIVE CONTEXT: ${tier === 'excellent' || tier === 'good' ? 'High awareness quality means the brand\'s aware consumer base converts to spontaneous consideration at a strong rate — the most commercially valuable form of brand equity, generating organic consideration without requiring paid media reminders at the moment of decision.' : 'The brand\'s aware base is underperforming commercially. Consumers who recognise the brand when shown it, but don\'t recall it without a prompt, have low conversion potential — they default to higher-quality awareness brands in most unaided purchase situations.'}`;

      const consumer = `CUSTOMER BEHAVIOUR: ${tier === 'excellent' || tier === 'good' ? 'Active recall correlates with higher purchase intent, greater loyalty, and stronger advocacy — making high-quality awareness a leading indicator of brand commercial value, not just market reach.' : 'The current consumer relationship is primarily recognitional. Converting passive recognisers requires deeper, more emotionally engaging brand touchpoints — not more reach, but more resonance and distinctiveness at every interaction.'}`;

      const risk = `KEY RISK AREAS: ${tier === 'excellent' || tier === 'good' ? 'Awareness quality can decline silently if brand distinctiveness erodes. A brand running undifferentiated creative grows total awareness while its quality ratio falls — more consumers know the name but fewer recall it first. This position is also well-suited to shift investment from awareness-building to preference-building.' : 'Low awareness quality means brand investment is generating diminishing returns: every additional aware consumer added without parallel quality improvement produces less commercial value. Improving quality does not require more reach investment; it requires better distinctiveness investment, typically at lower cost.'}`;

      const strategic = `BUSINESS IMPLICATION: ${tier === 'excellent' || tier === 'good' ? 'With strong awareness quality, look upstream (top-of-mind growth) and downstream (intent and loyalty conversion). Ensure the quality ratio stays above 25% as total awareness grows — or the brand risks accumulating passive awareness that doesn\'t convert.' : 'Improving awareness quality must become a primary brand objective. This means investing in distinctive brand assets — visual identity, tagline, brand characters — that create strong recall at every touchpoint. Quality improvement is measured not in more consumers knowing the brand, but in more aware consumers recalling it spontaneously.'}`;

      const action = `RECOMMENDED NEXT STEPS: ${tier === 'excellent' || tier === 'good' ? 'Monitor the quality ratio quarterly and set a floor (maintain above 30%). If quality declines while total awareness grows, investigate creative for distinctiveness erosion. Invest in brand consistency — inconsistent creative execution is the most common cause of quality ratio decline.' : 'Conduct a brand distinctiveness audit: which specific brand assets generate the strongest spontaneous recall? Build those assets into every creative execution at higher frequency and consistency. Set a 6-month quality improvement target (e.g. +5pp) before broadening reach investment.'}`;
    ...
    case 'share_of_voice': {
      ...
      const competitive = `COMPETITIVE CONTEXT: ${tier === 'commanding' || tier === 'strong' ? 'Strong SOV has a self-reinforcing quality: brands dominating first-mention moments gain disproportionate organic consideration, which reinforces salience, which generates more first-mention moments. This makes high-SOV positions defensible — challengers need sustained heavy investment to displace them.' : 'SOV is a relative metric — it reflects not just this brand\'s absolute performance but the competitive intensity of the market. Growing SOV through distinctive campaigns produces measurable market share gains within 12-18 months, making it one of the most evidence-based commercial investments.'}`;

      const consumer = `CUSTOMER BEHAVIOUR: ${tier === 'commanding' || tier === 'strong' ? 'Consumers naming this brand first spontaneously are demonstrating that it has become a category proxy — a mental shorthand for the entire banking category. This is the highest form of brand salience and the most reliable predictor of sustained market share.' : 'The brand\'s share of consumer first-mention is limited to a segment rather than the market-wide association that leading brands enjoy. It is prominent for some consumers but irrelevant to the majority who don\'t recall it first.'}`;

      const risk = `KEY RISK AREAS: ${tier === 'commanding' || tier === 'strong' ? 'SOV can be eroded by competitor investment increases even when this brand\'s absolute awareness is stable. Monitoring SOV quarterly is essential: a brand can lose mind-share even while its absolute top-of-mind holds steady, if competitors are growing faster. Dominant SOV also enables premium pricing and reduced promotional intensity.' : 'Small or moderate SOV leaves the brand exposed to SOV-driven growth by competitors. A challenger that doubles their share of first-mention moments will create measurable pressure on consideration within 2-3 quarters. SOV growth through distinctive campaigns produces market share gains with a relatively short lag.'}`;

      const strategic = `BUSINESS IMPLICATION: ${tier === 'commanding' || tier === 'strong' ? 'With strong SOV, focus should shift to deepening quality metrics (awareness quality, depth score) rather than protecting raw SOV share. A brand with 30% SOV and 40% awareness quality is far stronger commercially than one with 35% SOV and 15% quality.' : 'Research consistently shows brands with SOV above their market share tend to grow market share over time — this is the Excess Share of Voice principle. Setting an explicit SOV growth target and aligning media investment against it is one of the most evidence-based approaches to brand growth.'}`;

      const action = `RECOMMENDED NEXT STEPS: ${tier === 'commanding' || tier === 'strong' ? 'Track SOV quarterly against all major competitors. A sustained 3pp+ SOV decline over 2 consecutive quarters is a leading indicator of future market share erosion — it warrants immediate investigation and tactical response.' : 'Build a SOV growth plan: identify which competitor\'s share is most directly contestable and develop a targeted campaign strategy. Measure SOV monthly during active campaign periods to track response speed and adjust.'}`;
    ...
    case 'awareness_depth_score': {
      ...
      const competitive = `COMPETITIVE CONTEXT: ${tier === 'excellent' || tier === 'good' ? 'High depth scores provide competitive insulation in every unpropted purchase situation. Brands with higher depth scores win the consideration default whenever consumers make a decision without active comparison — the majority of routine financial choices.' : 'Competitors with higher depth scores enjoy a significant activation advantage in unaided purchase situations. Every interaction where a consumer isn\'t actively comparing — the majority of routine banking decisions — defaults to deeper-recall brands, representing a persistent conversion disadvantage.'}`;

      const consumer = `CUSTOMER BEHAVIOUR: ${tier === 'excellent' || tier === 'good' ? 'Consumers interacting with this brand come to it with a higher-quality mental relationship than those who only recognise it when shown it. High-depth consumers are more likely to seek out the brand, mention it in conversation, and choose it without extensive comparison shopping.' : 'The brand\'s consumer base includes a high proportion of "passive recognisers" — consumers who know the name but don\'t think of it when making a banking decision. Converting them to active recall requires more memorable brand touchpoints, not more reach.'}`;

      const risk = `KEY RISK AREAS: ${tier === 'excellent' || tier === 'good' ? 'Depth scores decline gradually as creative becomes less distinctive or as competitor investment increases. Monitor quarterly; a 5+ point drop in depth score over 2 periods warrants immediate creative review. High depth also enables a shift from reach investment to preference and loyalty investment.' : 'Brands with weak depth scores are structurally disadvantaged in any context where there is no external prompt. Depth improvement typically produces faster top-of-mind and SOV gains than awareness expansion, at lower cost, making it one of the best-ROI investments at this brand stage.'}`;

      const strategic = `BUSINESS IMPLICATION: ${tier === 'excellent' || tier === 'good' ? 'With strong depth, focus should shift to converting active recall into strong preference and advocacy. Investment should emphasise deepening the brand relationship with high-depth consumers rather than broadening reach to passive recognisers.' : 'Improving depth score must be a primary brand investment objective. Brands improve depth by being more memorable — not by reaching more consumers with the same message, but by embedding specific, distinctive brand cues consistently across every touchpoint.'}`;

      const action = `RECOMMENDED NEXT STEPS: ${tier === 'excellent' || tier === 'good' ? 'Track depth score monthly, especially during creative refresh periods — distinctiveness erosion is the most common cause of depth decline. Ensure brand asset guidelines (visual identity, brand characters) are strictly applied across all creative executions.' : 'Run a brand distinctiveness audit: identify the brand cues that most reliably drive unaided recall and concentrate investment on embedding them consistently. Set a 6-month depth score improvement target (e.g., +10 points) as a campaign success metric alongside reach metrics.'}`;
...
export function buildAwarenessFunnelInsight(args: FunnelInsightArgs): AwarenessInsightResult | null {
  ...
  const consumer = `CUSTOMER BEHAVIOUR: ${isWeakToM ? 'The brand is known but rarely the first recalled — consumers are processing it as a secondary option rather than a category anchor. In markets where choice is made quickly and instinctively, this "known but not first" position translates directly into lost conversion opportunities.' : isStrongToM ? 'The brand benefits from strong instinctive recall — a significant share of aware consumers name it first. This indicates the brand has moved beyond recognition into habitual salience, the highest form of consumer brand relationship.' : 'The aware base is split between active and passive recall. Active-recall consumers behave like brand advocates; passive-recall consumers behave like market browsers evaluating multiple options — a meaningful distinction for conversion rate expectations.'}`;

  const opportunityRisk = `KEY RISK AREAS: ${isWeakSpont ? 'Passive-heavy awareness is fragile: it generates no word-of-mouth, provides no competitive protection, and decays faster during media-dark periods. Investing in distinctiveness rather than more reach can shift this conversion ratio significantly within 2-3 campaign cycles, at lower cost than building new awareness.' : 'If funnel conversion is not actively managed, total awareness can grow while spontaneous and top-of-mind ratios decline — a "hollow awareness" pattern that reduces the commercial return on brand investment. The existing aware base is the most cost-efficient pool for salience conversion.'}`;
...
export function buildIntentInsight(args: IntentInsightArgs): AwarenessInsightResult | null {
  ...
  const hasStrongPipeline = highIntentNonUserPct > 25;
  const hasModeratePipeline = highIntentNonUserPct > 10 && !hasStrongPipeline;
  const hasChurnRisk = lowIntentCurrentUserCount > 10;
  const hasSubduedIntent = highIntentPct < 25;

  const snapshot = `Average intent of ${averageIntent.toFixed(1)}/10 among ${responseBase} aware respondents is ${intentLevel}.${hasStrongPipeline ? ` ${Math.round(highIntentNonUserPct)}% of non-users show high intent — strong acquisition pipeline.` : hasModeratePipeline ? ` ${Math.round(highIntentNonUserPct)}% of non-users show high intent — moderate acquisition opportunity.` : ''}${hasChurnRisk ? ` ${lowIntentCurrentUserCount} current users show low intent — material churn risk.` : ''}${hasSubduedIntent ? ' Overall intent is subdued.' : ''}`;

  const profile = `INTENT PROFILE: Average intent of ${averageIntent.toFixed(1)}/10 among ${responseBase} aware respondents is ${intentLevel}. ${Math.round(highIntentPct)}% of aware respondents score 7-10 (high intent). Intent is a forward-looking measure: it captures not how consumers feel about the brand today, but whether they plan to use or consider it — the earliest available signal of future consideration conversion.`;

  const pipeline = `ACQUISITION PIPELINE: ${hasStrongPipeline ? `${Math.round(highIntentNonUserPct)}% of non-users show high intent (7-10) — a strong, warm acquisition pipeline. These respondents are aware, interested, but not yet customers. They represent the highest-probability acquisition targets: they don't need awareness investment, they need a converting offer. Priority should be to activate this pipeline before it decays or competitor brands capture it.` : hasModeratePipeline ? `${Math.round(highIntentNonUserPct)}% of non-users show high intent — a moderate pipeline. These leads are warm; nurturing them through targeted consideration-stage content (product benefits, comparisons, proof points) should yield above-average conversion rates at lower cost than acquiring cold prospects.` : 'The acquisition pipeline among high-intent non-users is thin. Awareness investment alone is unlikely to drive new acquisition — consideration barriers need to be investigated: what is preventing aware, potentially interested non-users from moving toward the brand?'}`;
  ...
  const consumer = `CUSTOMER BEHAVIOUR: ${intentLevel === 'very high' || intentLevel === 'high' ? 'High average intent indicates that the aware consumer base is positively pre-disposed toward the brand — a strong foundation for conversion campaigns and loyalty investment. Aware consumers at this intent level are significantly closer to action than the market average.' : intentLevel === 'moderate' ? 'Moderate intent suggests a mixed consumer pre-disposition: some aware consumers are close to action, but a significant portion is neutral or passive. Understanding what drives the neutral cohort — price, trust, inertia, competitor preference — is the key diagnostic question.' : 'Low average intent among aware consumers is a serious signal: these respondents know the brand but are not moved toward it. This "aware but uninterested" cohort indicates either a product/service gap or an unresolved brand barrier that awareness investment cannot solve.'}`;
...
export function buildAwarenessModuleSummary(payload: AwarenessReportPayload): AwarenessInsightResult | null {
  ...
  if (hasLowAwareness && hasStrongToM) {
    ...
    primaryAction = 'Commission a brand distinctiveness audit. Identify which brand assets most reliably generate spontaneous recall in this category and embed them at higher frequency and consistency across all creative. Set a 6-month awareness quality improvement target (e.g. +5pp).';
  } else if (hasLowAwareness && hasWeakQuality) {
