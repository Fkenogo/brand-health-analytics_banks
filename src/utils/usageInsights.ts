...
export function buildPreferenceInsight(args: {
  ...
  if (tier === 'shallow') {
    ...
    bumo = `BUMO PENETRATION CONTEXT: BUMO (best-used most-often) penetration of ${fmtPct(bumoPenetration)} — preferred users as a share of the aware market — confirms the structural preference gap. The bank is not winning the primary-bank decision in the broader market either, only holding accounts within active users.`;
    ...
    bumo = `BUMO PENETRATION CONTEXT: BUMO penetration of ${fmtPct(bumoPenetration)} — preferred users as a share of the aware market — indicates the bank holds primary status for a meaningful but minority slice of the addressable market. Growth in BUMO penetration is one of the most direct predictors of category share gain.`;
    ...
    bumo = `BUMO PENETRATION CONTEXT: BUMO penetration of ${fmtPct(bumoPenetration)} — preferred users as a share of the aware market — shows the bank captures primary-bank status at a structurally advantaged rate across the whole addressable market, not just within its existing relationships.`;
